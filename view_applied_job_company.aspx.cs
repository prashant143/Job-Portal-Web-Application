...
public partial class view_applied_job_company : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (Session["employer"] != null)
            {
                ...
                lblmessage.Text = "Job Applied Detail for " + Session["employer"] + "Company";
                if (!IsPostBack)
                {
                    bind();
                    job();
    ...
    public void bind()
    {
        try
        {
            ...
            //string str = "select a.jobapplied_id,j.job_title,j.interview_date,d.username,r.path from job_post j,job_applied a,jobseeker_detail d,resume r where j.company_id='" + id + "' and a.jobpost_id=j.jobpost_id and d.jobseeker_id=a.jobseeker_id and d.jobseeker_id=r.jobseeker_id and j.interview_date>='" + da + "' a.jobseeker_id not in(select jobseeker_id from save_candidates)";
           
            ds = obj.conne("select a.jobapplied_id,j.job_title,j.interview_date,d.username,r.path from job_post j,job_applied a,jobseeker_detail d,resume r where j.company_id='" + id + "' and a.jobpost_id=j.jobpost_id and d.jobseeker_id=a.jobseeker_id and d.jobseeker_id=r.jobseeker_id and j.interview_date>='" + da + "'");
    ...
    public void job()
    {
        ds = obj.conne("select job_title from job_post where company_id=(select company_id from company where username='"+ Session["employer"] +"') group by job_title");
    ...
    protected void grdapplyjob_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            id1 = obj.nubmerofrow("select jobseeker_id from job_applied where jobapplied_id='"+ id +"'");
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ds = obj.conne("select a.jobapplied_id,j.job_title,j.interview_date,d.username,r.path from job_post j,job_applied a,jobseeker_detail d,resume r where j.company_id='" + id + "' and a.jobpost_id=j.jobpost_id and d.jobseeker_id=a.jobseeker_id and d.jobseeker_id=r.jobseeker_id and j.interview_date>='" + da + "' and j.job_title='" + ddljob.SelectedItem + "'");
            }
            grdapplyjob.DataSource = ds;
            grdapplyjob.DataBind();
        }
